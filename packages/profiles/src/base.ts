...
/**
 * Coerce any {@link BufferSource} into a `Uint8Array` view over its bytes,
 * preserving the source's `byteOffset` / `byteLength` window. Shared by the
 * serial-pipe profiles (Nordic UART, HM-10) that chunk an outbound payload.
 */
export function toBytes(value: BufferSource): Uint8Array {
  if (value instanceof ArrayBuffer) {
