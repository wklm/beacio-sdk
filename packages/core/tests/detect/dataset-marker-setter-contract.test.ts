...
// detect now lives inside @beacio/core: packages/core/tests/detect -> src/detect.
const INSTALL_STATE_SRC = path.resolve(__dirname, '..', '..', 'src', 'detect', 'install-state.ts');
// The content script lives in the EXTENSION source tree (repo root src/extension),
// four levels up from packages/core/tests/detect.
