...
// SB-SDK-02 (Part B): @beacio/core is an OPTIONAL peer (it may be absent on a
// standalone `npm i @beacio/detect`), so it must NOT be hard-imported at module
// top level — that throws at load. The canonical /setup URL lives in core
// (urls.ts) for the dispatcher/react-sdk to share, but the value is inlined here
// and pinned to core's type via a fully-erased `typeof import(...)` query: if
// core's SETUP_URL literal ever changes, this assignment becomes a COMPILE error,
// so the constant cannot silently drift while no runtime core load is required.
const SETUP_URL: typeof import('@beacio/core').SETUP_URL = 'https://beacio.com/setup';
...
// SB-SDK-12: isExtensionActiveNow / isDismissed / setDismissed / saveReturnContext
...
#bc-s{background:#fff;border-radius:16px 16px 0 0;padding:12px 24px 34px;max-width:420px;
  width:100%;animation:bc-su .3s ease-out;max-height:90vh;overflow-y:auto}
#bc-s *{box-sizing:border-box;margin:0;padding:0}
.bc-h{width:36px;height:5px;border-radius:3px;background:#d1d1d6;margin:0 auto 16px}
.bc-hdr{display:flex;align-items:center;gap:12px;margin-bottom:12px}
.bc-ic{width:40px;height:40px;border-radius:10px;background:var(--bc-accent);display:flex;
  align-items:center;justify-content:center;flex-shrink:0;overflow:hidden}
.bc-ic svg{width:22px;height:22px;fill:#fff}
.bc-ic img{width:100%;height:100%;object-fit:contain}
.bc-tt{font-size:17px;font-weight:600;color:#000}
.bc-bd{font-size:15px;line-height:1.4;color:#8e8e93;margin-bottom:16px}
.bc-steps{list-style:none;margin:0 0 18px;padding:0;counter-reset:bc-step}
.bc-step{position:relative;padding:0 0 12px 30px;font-size:14px;line-height:1.4}
.bc-step::before{counter-increment:bc-step;content:counter(bc-step);position:absolute;left:0;top:0;
  width:20px;height:20px;border-radius:50%;background:var(--bc-accent);color:#fff;font-size:12px;
  font-weight:600;display:flex;align-items:center;justify-content:center}
.bc-step-l{display:block;font-weight:600;color:#1c1c1e}
.bc-step-w{display:block;color:#8e8e93;margin-top:2px}
.bc-btn{display:block;width:100%;padding:14px;background:var(--bc-accent);color:#fff;border:none;
  border-radius:12px;font-size:17px;font-weight:600;cursor:pointer;text-align:center;
  text-decoration:none;-webkit-tap-highlight-color:transparent}
.bc-btn:active{opacity:.85}
.bc-ret{display:block;width:100%;padding:14px;margin-top:10px;background:#34c759;color:#fff;border:none;
  border-radius:12px;font-size:17px;font-weight:600;cursor:pointer;text-align:center;
  text-decoration:none;-webkit-tap-highlight-color:transparent}
.bc-ret:active{opacity:.85}
.bc-cb{font-size:12px;color:#8e8e93;text-align:center;margin-top:8px}
...
.bc-privacy{font-size:13px;color:#8e8e93;line-height:1.5;margin-top:16px}
.bc-noaff{font-size:12px;color:#8e8e93;line-height:1.4;margin-top:8px;text-align:center}
.bc-det{margin-top:16px}
.bc-det summary{font-size:15px;color:var(--bc-accent);cursor:pointer;list-style:none;padding:4px 0}
.bc-det summary::before{content:'\\25B8  '}
.bc-det[open] summary::before{content:'\\25BE  '}
.bc-det p{font-size:13px;color:#8e8e93;line-height:1.5;padding:8px 0 4px}
.bc-det a{color:var(--bc-accent)}
.bc-stuck{display:block;font-size:13px;color:var(--bc-accent);text-align:center;margin-top:14px;
  text-decoration:none}
.bc-reload{display:block;width:100%;padding:13px;margin-top:10px;background:none;
  border:1px solid var(--bc-accent);border-radius:12px;font-size:16px;font-weight:600;color:var(--bc-accent);
  cursor:pointer;text-align:center;-webkit-tap-highlight-color:transparent}
.bc-reload:active{opacity:.7}
.bc-dis{display:block;width:100%;padding:12px;background:none;border:none;font-size:15px;
...
.bc-dont{font-size:13px;padding:6px 12px;margin-top:0}
