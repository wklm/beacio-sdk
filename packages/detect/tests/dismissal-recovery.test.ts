 ...
 *    from the long dismiss(days) window, and the long path / setDismissed alias is
...
import { removeInstallBanner, showInstallBanner } from '../src/banner';
import {
  dismiss,
  dismissShort,
  isDismissed,
...
import { EN_STRINGS } from '../src/i18n';
...
describe('SB-PRD-08 soft/hard dismissal split + force-show recovery', () => {
  ...
  it('back-compat: setDismissed is still the long dismiss() alias (unchanged)', () => {
    expect(setDismissed).toBe(dismiss);
    setDismissed(14);
...
describe('SB-PRD-08 AC4: S&B demo "Can\'t connect?" link is repointed (editable fork only)', () => {
  ...
  it('the #cantConnectLink re-invokes the in-app beacio activation flow', () => {
    ...
    const linkLine = html
      .split('\n')
      .find((l) => l.includes('id="cantConnectLink"'));
